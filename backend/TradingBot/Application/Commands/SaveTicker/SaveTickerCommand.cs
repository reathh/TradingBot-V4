        ...
        ILogger<SaveTickerCommandHandler> logger) : BaseCommandHandler<SaveTickerCommand>(logger)
    {
        protected override async Task<Result> HandleCore(SaveTickerCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                logger.LogDebug("Saving ticker data for {Symbol}: Bid={Bid}, Ask={Ask}, Last={Last}", 
                    tickerDto.Symbol, tickerDto.Bid, tickerDto.Ask, tickerDto.LastPrice);

                var ticker = new Ticker(tickerDto);
                
                await dbContext.Tickers.AddAsync(ticker, cancellationToken);
                await dbContext.SaveChangesAsync(cancellationToken);
                
                logger.LogDebug("Successfully saved ticker data for {Symbol} at {Timestamp}", tickerDto.Symbol, tickerDto.Timestamp);
                
                return Result.Success;
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Error saving ticker data");
